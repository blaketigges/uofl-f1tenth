...
#Used to drive via maparea function 
    def mapping_callback(self,msg):
        ...
        if (surrounding.ObjectFrontDistance > SlowDownDistance ):
            print(f"Closest object in front {0}, driving straight", surrounding.ObjectFrontDistance)
            drive_msg = AckermannDriveStamped()
            drive_msg.header.stamp = rospy.Time.now()
            drive_msg.header.frame_id = "laser"
            drive_msg.drive.steering_angle = 0 
            if (self.CurSpeed <= MaxSpeed):
                self.CurSpeed += 0.2
            print("Current Speed: ", self.CurSpeed)
            drive_msg.drive.speed = self.CurSpeed
            self.drive_pub.publish(drive_msg)   
        elif(surrounding.ObjectFrontDistance > StoppingDistance ):
            print(f"Closest object in front {0}, driving straight", surrounding.ObjectFrontDistance)
            drive_msg = AckermannDriveStamped()
            drive_msg.header.stamp = rospy.Time.now()
            drive_msg.header.frame_id = "laser"
            ...
            if (self.CurSpeed > DefaultDriveSpeed):
                self.CurSpeed -= 0.01
            elif(self.CurSpeed < DefaultDriveSpeed):
                self.CurSpeed = DefaultDriveSpeed
            print("Current Speed: ", self.CurSpeed)
            ...
            if(turn == TurnRight):
                print(f"Closest object to Right {0}, Turning Right", surrounding.ObjectRightDistance)
                ...
                print(f"Closest object to left {0}, Turning Left", surrounding.ObjectRightDistance)
