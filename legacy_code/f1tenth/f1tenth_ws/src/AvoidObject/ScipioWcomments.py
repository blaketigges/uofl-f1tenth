...
class ObjectAvoidance:
    """ Implement Object Avoidance algorithm
    """
    PreviousTime = 0
    CurSpeed = 0.5
    def __init__(self):
...
##Similiar process to MapArea but used for scan_callback 
...
##decide to turn left, turn right, or turn around 
    ...
    def Deaccelerate (self, targetspeed):
        ...
        if (self.CurSpeed >= targetspeed):
            self.CurSpeed -= .1
            return self.CurSpeed
        #Commented logic below does not work as expected 
        """if (self.CurSpeed >= targetspeed):
            current_time = rospy.get_time()
    ...
    #Should drive straight if no object in front, will accelerate
    #to max speed as long as the closest object in front is greater than the 
    # GoodToAccelerate constant specified distance.  If vehicle is not good to accelerate it 
    # will deaccelerate till default speed, if it is in danger of hitting a wall without turning 
    # in time it will stop.  If there is space to still turn, it will go to turning 
    # decision function, and can turn Left, Slight Left, Right, Slight Right.  
    # If it can move forward or turn in either direction, it will acknowledge that it needs to 
    # turn aroudnd but logic for that is still on the TODO 
...
#Scan callback is a simple autonomous driving methodology 
#used to make the vehcicle drive straight while no object is detected in front 
#if an object is in front, then stop and turn wheels.  
    def scan_callback(self, msg):
        surroundingCheck = self.DetectSurroundings(msg)
        print(surroundingCheck)
        #if no object in front, drive straight
        if (surroundingCheck == NoObject):
            ...
            drive_msg.drive.speed = .500
            self.drive_pub.publish(drive_msg)  
        #if object in front, turn wheels                  
        else:
            drive_msg = AckermannDriveStamped()
            drive_msg.header.stamp = rospy.Time.now()
            drive_msg.header.frame_id = "laser"
